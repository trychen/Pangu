 ...
 * cached, it will remain stored in an OpenGL texture for the entire lifetime of the application (FontCache depends on this
 * behavior).
 *
 ...
 * and for use in the F3 debug screen; may need some explicit argument in FontCache.renderString() to select the size
...
public class GlyphCache {
    ...
    /**
     * A single instance of GlyphCache is allocated for internal use by the FontCache class.
     */
    public GlyphCache() {
     ...
     * in their respective textures and will remain accessible to FontCache. This method is normally called by FontCache.setDefaultFont() since
     * the FontCache must also invalidate itself so it can re-layout and re-cache the glyphs for all strings using the new font.
     *
    ...
    private void allocateGlyphCacheTexture() {
        ...
        this.texture = PanguResLoc.of("text." + this.textureName);
