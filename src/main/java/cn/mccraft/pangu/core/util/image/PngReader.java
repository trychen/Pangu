...
import com.googlecode.pngtastic.core.*;
...
public class PngReader extends PngProcessor {
    public static final PngReader INSTANCE = new PngReader();
    ...
    public int[] readARGB8(final PngImage image) throws IOException {
        log.debug("=== READING ===");
        ...
        final int[] argb = getARGB8(image, originalScanlines);

        return argb;
    }

    private int[] getARGB8(final PngImage original, final List<byte[]> rows) throws IOException {
        ...
        int[] result = new int[perRow * rows.size()];
        int y = 0;
        for (byte[] row : rows) {
                ...
                switch (imageType) {
                    ...
                    case GREYSCALE_ALPHA: {
                        if (original.getBitDepth() == 8) {
                            ...
                            result[index] = ((Integer.divideUnsigned(a, max) + (Integer.remainderUnsigned(a, max) > max / 2 ? 1 : 0) & 0xFF) << 24) |
                                    ((v & 0xFF) << 16) |
                                    ((v & 0xFF) << 8)  |
                                    ((v & 0xFF));
                            ...
                            result[index] = (((Integer.divideUnsigned(a, 65535) + (Integer.remainderUnsigned(a, 65535) > 32767 ? 1 : 0)) & 0xFF) << 24) |
                                    ((v & 0xFF) << 16) |
                                    ((v & 0xFF) << 8)  |
                                    ((v & 0xFF));
                        }
                    ...
                    case TRUECOLOR: {
                        if (original.getBitDepth() == 8) {
                            ...
                            result[index] = (0xFF << 24) |
                                    (((Integer.divideUnsigned(r, 65535) + (Integer.remainderUnsigned(r, 65535) > 32767 ? 1 : 0)) & 0xFF) << 16) |
                                    (((Integer.divideUnsigned(g, 65535) + (Integer.remainderUnsigned(g, 65535) > 32767 ? 1 : 0)) & 0xFF) << 8)  |
                                    (((Integer.divideUnsigned(b, 65535) + (Integer.remainderUnsigned(b, 65535) > 32767 ? 1 : 0)) & 0xFF));
                        }
                        break;
                    }
